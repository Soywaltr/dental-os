...
// Iconos vectoriales suaves y modernos (Sin emojis)
const ICONS = {
...
export default function App() {
  ...
  return (
    // FONDO INMERSIVO GLOBAL ESTILO "HEAL.ME"
    <div style={{ 
      ...
      <div style={{ padding: '24px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 50, flexShrink: 0 }}>

        {/* LOGO IZQUIERDA */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={() => goTo('dashboard')}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5z"></path><path d="M2 17l10 5 10-5"></path><path d="M2 12l10 5 10-5"></path></svg>
          </div>
          <span style={{ fontSize: 22, fontWeight: 800, color: '#0F172A', letterSpacing: '-0.5px' }}>DentalOS</span>
        </div>

        {/* MENÚ CENTRAL (Pill Nav Estilo heal.me) */}
        <div style={{ background: '#FFFFFF', padding: '6px', borderRadius: '100px', display: 'flex', gap: 4, boxShadow: '0 10px 25px rgba(0,0,0,0.03)' }}>
          {NAV.map(it => {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ position: 'relative', width: '260px' }}>
            <svg style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: '#94A3B8' }} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
            <input type="text" placeholder="Search" style={{ width: '100%', padding: '12px 16px 12px 44px', borderRadius: '100px', border: 'none', background: '#FFFFFF', fontSize: '14px', outline: 'none', boxSizing: 'border-box', fontWeight: 500, color: '#0F172A', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }} />
          </div>

          {/* Settings Icon */}
          <div onClick={() => goTo('config')} style={{ width: 44, height: 44, borderRadius: '50%', background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#475569', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
          ...
          {/* Notification Bell */}
          <div style={{ position: 'relative', width: 44, height: 44, borderRadius: '50%', background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#475569', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg>
          </div>

          {/* Profile Avatar */}
          <div onClick={handleLogout} style={{ width: 44, height: 44, borderRadius: '50%', background: '#E2E8F0', backgroundImage: 'url(/drasolvargas.jpeg)', backgroundSize: 'cover', cursor: 'pointer', border: '2px solid #fff', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }} title="Cerrar sesión" />
        </div>

      </div>

      {/* ─── CONTENIDO DINÁMICO (Sin la caja blanca, directo al fondo) ─── */}
